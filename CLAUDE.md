...
Switches every 21-derived example honours (21-31): `HYDROPT_SONAR` (`120`,
...
(30's and 31's fits).  22-31 lay their scenes out for the 120 kHz head's 300 m
...
`fit_scene` on `range_profile`, the level against range; `30`), and
`SceneFit.renderer()` makes the fitted scene a `PictureRenderer` for the
boat's fit on it (`31`: scene, detection, heading by search, position by
descent with the loss restricted to a box round the detection).
...
* **21-31 share one scene.**  22-31 import `21_long_range_300m.py` for the
...
* **Beads (`bd`) for task tracking** where the tool exists, `bd remember`
  ...
  **Without `bd`, `HANDOFF.md` is the task list and the memory**: append
  status to its tasks and add a lesson to its last section when you learn
  one, and keep this file's rules in step.  No markdown TODOs elsewhere.
...
* A boat on a background the model does not share (`31`): the heading
  from the echo's principal axis (1.3 deg), then a SCAN at half a
  patch-lobe's step (`lambda / 5 m`, 0.14 deg at 120 kHz) about it --
  a 1.3 deg error lights different patches, quadruples the loss and
  leaves the position 3.6 m off; the scan gets 0.16 deg and 0.15 m.
  Coarser heading searches by the loss walk AWAY (15, 5 and 2.5 deg steps
  all did): wider than the lobe they see only the ripple's envelope.  The
  loss is restricted to a DISC round the centroid that holds the hull
  whichever way it lies (a box one beam wide clipped it at 330 kHz), and
  the coarse stages blur wider than the 5 m bead spacing (8 m).
...
**If you do not have the `bd` tool, `HANDOFF.md` at the repository root is
the task list and the memory: every open task with its acceptance criteria
and every stored lesson, in full.  Work from it and append your status to
each task there.**  Everything is committed on
`claude/hydropt-acoustic-tracer-15kecr`; the suite is green (574); examples
01-29 pass at 120 kHz and 21-29 at 330 kHz; the figures are regenerated by
a run.  The next pieces, in the order to take them (their full text is in
`HANDOFF.md`, and in `bd show <id>` where `bd` exists):
...
Read the lessons first (`HANDOFF.md`'s last section, or `bd prime`); each
one is a day that need not be repeated.
